...
draws <- draws %>%
  pivot_longer(names_to = "state", values_to = "trump_share", 
               cols = Alabama:Wyoming)

draws %>%
  filter(state %in% c("Wisconsin", "Indiana")) %>%
  ggplot(aes(x = trump_share, color = state)) + 
    geom_density() +
    labs(x = "Trump vote percentage") +
    facet_wrap(~state)
...
```{r}
elections <- tibble(number = rep(1, 1000)) %>%
  mutate(simulations = map(number, ~simulate_election(n_times = .))) %>%
  select(-number) %>%
...
```{r}
elections2 <- tibble(number = rep(1, 1000)) %>%
  mutate(simulations = map(number, ~simulate_election(n_times = .))) %>%
  select(-number) %>%
  mutate(pa_win = map_lgl(simulations, ~.$Pennsylvania > 50),
         oh_win = map_lgl(simulations, ~.$Ohio > 50),
         both = oh_win == TRUE & pa_win == TRUE) %>%
  summarize(pa_avg = mean(pa_win), oh_avg = mean(oh_win), both_avg = mean(both))
...
elections3 <- tibble(number = rep(1, 1000)) %>%
  mutate(simulations = map(number, ~simulate_election(n_times = .))) %>%
  select(-number) %>%
  ...
  group_by(pa_win) %>%
  summarize(oh_win_avg = mean(oh_win), .groups = "drop")
...
elections3 <- tibble(number = rep(1, 1000)) %>%
  ...
  mutate(pa_win = map_lgl(simulations, ~.$Pennsylvania > 50),
         oh_win = map_lgl(simulations, ~.$Ohio > 50)) %>%
  group_by(oh_win) %>%
  summarize(mean_pa = mean(pa_win), .groups = "drop")
